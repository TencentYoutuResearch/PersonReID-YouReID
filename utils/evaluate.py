...
def eval_part_result(info, use_rerank=False, use_pcb_format=True, logger=None):

    gallery = info['gallery']
    query = info['query']

    dist = utils.metric.part_distance(query, gallery)
    if use_rerank:
        qq_dis = utils.metric.part_distance(query, query)
        gg_dis = utils.metric.part_distance(gallery, gallery)
