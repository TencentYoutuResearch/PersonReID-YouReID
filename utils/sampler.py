...
class DistributeRandomIdentitySampler(Sampler):
    ...
    def __init__(self, data_source, batch_size, num_instances, use_tf_sample=False, use_all_sample=False, shuffle=True, seed=0):
        if batch_size < num_instances:
            raise ValueError('batch_size={} must be no less '
                             'than num_instances={}'.format(batch_size, num_instances))
    ...
    def __iter__(self):
        ...
        if not self.use_tf_sample:
            ...
            if not self.use_all_sample:
                while len(avai_pids) >= self.num_pids_per_batch:
                    selected_pids = np.random.choice(avai_pids, size=self.num_pids_per_batch, replace=False)
                    for pid in selected_pids:
                        batch_idxs = batch_idxs_dict[pid].pop(0)
...
class RandomCameraSampler(Sampler):
    ...
    def __init__(self, data_source, batch_size, num_instances):
        ...
        self.num_pids_per_batch = self.batch_size // self.num_instances
        self.index_dic = defaultdict(list)
        for index, (_, _, cid) in enumerate(self.data_source.imgs):
