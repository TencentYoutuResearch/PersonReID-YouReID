...
model_urls = {
    'resnext101_ibn_a': os.path.expanduser('~/.cache/torch/hub/checkpoints/resnext101_ibn_a.pth.tar'),
}
...
class ResNeXt_IBN(nn.Module):
    ...
    def __init__(self,
                 ...
                 layers,
                 ibn_cfg=('a', 'a', 'a', None),
                 num_classes=1000, last_stride=1):
...
def resnext101_ibn_a(pretrained=False, baseWidth=4, cardinality=32, last_stride=1, **kwargs):
    """
    Construct ResNeXt-101-IBN-a.
    """
    model = ResNeXt_IBN(baseWidth, cardinality, [3, 4, 23, 3], ('a', 'a', 'a', None), last_stride=1, **kwargs)
    if pretrained:
        if 'LOCAL_RANK' in os.environ and os.environ['LOCAL_RANK']:
            print('map weight to cuda: %s' % str(os.environ['LOCAL_RANK']))
            state_dict = torch.load(model_urls['resnext101_ibn_a'], map_location="cuda:" + str(os.environ['LOCAL_RANK']))
        else:
            state_dict = torch.load(model_urls['resnext101_ibn_a'])
