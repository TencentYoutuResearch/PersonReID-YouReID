...
def eval_result_mask(data, root, use_metric_cuhk03=False,
    ...
    dist = utils.metric.mask_distance(query, gallery)
    if use_rerank:
        qq_dis = utils.metric.mask_distance(query, query)
        gg_dis = utils.metric.mask_distance(gallery, gallery)
    ...
    cmc_scores, mAP = utils.measure.evaluate_rank(dist, np.array(query_label), np.array(gallery_label),
                                                  np.array(query_cam), np.array(gallery_cam), max_rank=50,
