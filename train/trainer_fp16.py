        ...
        test_loader = {
            'test':
                torch.utils.data.DataLoader(
                    dataset.__dict__['TestData'](root=cfg['root'], dataname='test',
                                                        require_path=True,
                                                        size=(cfg['height'], cfg['width']),
                                                        **params
                                                        ),
        ...
        with torch.no_grad():
            ...
            for i, (input, target, path) in enumerate(val_loader):
                ...
                if isinstance(outputs, list):
                    feat = normalize(torch.cat(outputs[1], dim=1), axis=1)
                else:
...
def convert_to_onnx(model, test_loader):
    ...
    for p, val_loader in test_loader.items():
        for i, (input, target, path) in enumerate(val_loader):
            input = input.cuda(non_blocking=True)
            target = target.cuda(non_blocking=True)
            # print(model.resnet.bn1.bn_list[0].bias.cpu().numpy())
            torch.onnx.export(model, input, os.path.join(config.get('task_id'), '%s.onnx' % os.path.basename(config.get('task_id'))),
...
def save_feature(part, data, features, labels, paths):
    if not os.path.exists(config.get('task_id')):
        os.makedirs(config.get('task_id'))
    print('save at ', os.path.join(config.get('task_id'), data +'_'+ part+'.mat'))
