...
def bulid_dataset():
    ...
    train_dataset = dataset.__dict__[cfg['train_data_name']](root=cfg['root'], dataname=cfg['train_name'], part='train',
    ...
    test_loader = {
        'query':
            torch.utils.data.DataLoader(
                dataset.__dict__[cfg['test_data_name']](root=cfg['root'], dataname=cfg['test_name'], part='query',
        ...
        'gallery':
            torch.utils.data.DataLoader(
                dataset.__dict__[cfg['test_data_name']](root=cfg['root'], dataname=cfg['test_name'],
...
def train(scaler, train_loader, model, optimizer, lr_scheduler, epoch):
    ...
    for i, (input_0, input_1) in enumerate(train_loader):
        # measure data loading time
        data_time = time.time() - end

        lr_scheduler.step(epoch + float(i) / len(train_loader))
        input_0 = input_0.cuda(non_blocking=True)
        input_1 = input_1.cuda(non_blocking=True)
        input = torch.cat([input_0, input_1])
        # compute output
        ...
        with torch.cuda.amp.autocast():
            output = model(input)
            ce_losses, tri_losses = model.module.compute_loss(output)
            ce_loss = torch.sum(torch.stack(ce_losses, dim=0))
            # tri_loss = torch.summ(torch.stack(tri_losses, dim=0))
