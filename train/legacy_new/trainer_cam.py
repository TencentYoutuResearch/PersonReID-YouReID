...
def bulid_dataset():
    ...
    params = {'logger': logger, 'return_cam': True}
...
def train(scaler, train_loader, model, optimizer, lr_scheduler, epoch):
    ...
    for i, (input, target, cam) in enumerate(train_loader):
        ...
        cam = cam.cuda(non_blocking=True)
        # compute output
        ...
        with torch.cuda.amp.autocast():
            output = model(input)
            ce_losses, tri_losses, cam_losses = model.module.compute_loss(output, target, cam)
            ce_loss = torch.sum(torch.stack(ce_losses, dim=0))
            tri_loss = torch.sum(torch.stack(tri_losses, dim=0))
            cam_loss = torch.sum(torch.stack(cam_losses, dim=0))
            loss = ce_loss + tri_loss + cam_loss  # args.weight*
        ...
        if i % config.get('print_freq') == 0:
            ...
            for cam_id, ca in enumerate(cam_losses):
                show_loss += 'CamTri_%d: %f ' % (cam_id, ca.item())
