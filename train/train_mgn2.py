...
def train(train_loader, model, criterion, tri_criterion, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        ce_loss = torch.sum(torch.stack([criterion(logit, target) for logit in outputs[9:]], dim=0))
        tri_loss = torch.sum(torch.stack([tri_criterion(feat.renorm(2, 0, 1e-5).mul(1e5), target) for feat in outputs[1:9]], dim=0))
        ...
        prec1, prec5 = accuracy(outputs[9], target, topk=(1, 5))
