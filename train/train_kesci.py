...
def main():
    ...
    train_sampler = RandomIdentitySampler(data, args.batch_size, 4)
    ...
    model = net(class_num=data.class_num, pretrained=args.pretrained)
    ...
    tri_criterion = loss.TripletLoss().cuda()
    ...
    lr_scheduler = WarmupMultiStepLR(optimizer, [30, 50], warmup_iters=10, warmup_factor=1.0, gamma=0.1, last_epoch=-1)
    ...
    if args.evaluate:
        ...
        evaluate.eval_kesci(code=args.code, data=args.data, rerank=args.rerank)
        return
...
def train(train_loader, model, criterion, tri_criterion, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        # compute output
        logit, feat, _ = model(input)
        if type(logit) is list:
            feat = torch.cat(feat, dim=1)
            loss = torch.sum(torch.stack([criterion(l, target) for l in logit], dim=0)) + tri_criterion(feat, target)
        else:
            loss = criterion(logit, target) + tri_criterion(feat, target)


        # measure accuracy and record loss
        if type(logit) is list:
            prec1, prec5 = accuracy(logit[0], target, topk=(1, 5))
        else:
