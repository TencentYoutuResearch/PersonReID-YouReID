...
def main():
    ...
    # Data loading code
    data = dataset.__dict__[args.data](part='train', size=(args.height, args.width), use_random_pad=args.use_random_pad,
...
def train(lr_scheduler, train_loader, model, criterion, tri_criterion, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        global_softmax, global_triplet = model(input)
        losses, losses_name = [], []
        flags = [0]
    ...
    for p, val_loader in test_data.items():
        ...
        with torch.no_grad():
            ...
            for i, (input, target, path) in enumerate(val_loader):
                ...
                # compute output
                outputs = model(input)
                feats = normalize(outputs[1][0])

                input_ = input.flip(3)
                outputs = model(input_)
                feats_ = normalize(outputs[1][0])
